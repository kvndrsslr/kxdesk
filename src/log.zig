...
/// differs, so a repeating failure logs once until it clears. An empty message
/// is never a change. Fixed 160-byte memory, truncating longer messages.
pub const Once = struct {
    last: [160]u8 = undefined,
    last_len: usize = 0,

    pub fn changed(self: *Once, message: []const u8) bool {
        // An empty message is nothing to report and nothing to remember.
