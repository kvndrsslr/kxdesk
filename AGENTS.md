...
- Quick access terminals: `kxdesk term toggle <name>` (`src/kitty.zig`) — the terminal is `<kitty config dir>/quick-access-terminals/<name>.conf` merged over the shared `quick-access-terminal-base.conf`, and the toggle is a kitty remote control message to the socket the terminal was started with (`/tmp/kxdesk-qat-<uid>/<name>-<pid>`, which is why the sockets are looked for rather than remembered). Only a terminal that is not running costs a `kitten quick_access_terminal` process, and it is started hidden and then shown. Two clicks run the same path as background tasks (`dispatch.zig:load_terminal`, `github_terminal`): the load graphs toggle `btop`, the GitHub bell `ghr`. The base unbinds the tab/window/split shortcuts, so a terminal is one window.
...
- Subsystems: `src/store.zig` (SQLite), `src/yabai.zig` + `src/yabai_ops.zig` (yabai queries, and the `wm` verb table), `src/pomodoro.zig`, `src/zen.zig`, `src/mode_indicator.zig`, `src/server_mode.zig`, `src/kitty.zig` (the quick access terminals), `src/background.zig`, `src/exec.zig`, `src/app_icons.zig`.
